...
/// 계정 전환이 **호스트 앱(PokeTokenBar)** 에 남기는 뒤처리의 판정.
///
/// 전환 자체는 `AccountsState` 가 하지만, 그 결과로 낡아지는 것은 PokeTokenBar 쪽 캐시다 —
/// 두 세계가 서로를 모르는 채로 두기 위해 판정만 여기 값으로 꺼내 두고, 실행은 둘 다 들고 있는
/// `AppDelegate` 가 한다.
enum MobiusSwitchSideEffects {
    /// 이 전환이 PokeTokenBar 의 Claude 자격증명 캐시(`OAuthAccessTokenCache`)를 낡게 만드는가.
    ///
    ...
    ///
    /// Codex 는 해당 없다: PokeTokenBar 의 Codex 한도는 `~/.codex/sessions` 로그에서 읽으므로

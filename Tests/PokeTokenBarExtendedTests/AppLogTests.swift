...
final class AppLogTests: XCTestCase {
    ...
    func testAppLogWriteAndFlushRoutesThroughBackend() throws {
        let source = try String(contentsOf: repositoryFile("Sources/PokeTokenBarExtended/Core/AppLog.swift"))
    ...
    func testMarkCleanUsesWriteAndFlushNotWriteAlone() throws {
        let source = try String(contentsOf: repositoryFile("Sources/PokeTokenBarExtended/Core/CrashReporter.swift"))
        let body = try XCTUnwrap(
    ...
    func testDuplicateYieldUsesWriteAndFlushBeforeTerminate() throws {
        let source = try String(contentsOf: repositoryFile("Sources/PokeTokenBarExtended/PokeTokenBarExtendedApp.swift"))
        let body = try XCTUnwrap(
